...
//by aswathy
- (void)singleTapForGoToViewtouchIndex:(NSInteger)touch_index touchCell:(SCHCircleViewCell *)cell
{
...
- (void)singletTapFromIconCircleDefault:(NSMutableArray *)array touchCell:(SCHCircleViewCell *)cell touchIndex:(NSInteger)touch_index
{
    /*点击的 最前展示的*/
    if(touch_index == _current_index)
    {
...
- (CGFloat)getScaleByRadina:(CGFloat)radian originScale:(CGFloat)origin_scale deviationRadian:(CGFloat)d_radina
{
    ...
    CGFloat change_scale = 1.0;  //1.4f - (1.0f - origin_scale) * (change_radin /  M_PI); // aswathy  //1.0f - (1.0f - origin_scale) * (change_radin /  M_PI);
    // NSLog(@"%f",change_scale); // for no animations change it to 1.0
    return change_scale;
...
- (void)load
{
    ...
    if(_number_of_circle_cell > 0)
    {
        ...
        for (int i = 0; i < _number_of_circle_cell; ++i)
        {
            ...
            float viewHeight = ScreenFrame.size.height;
            
            // iOS 7 fix : Abdu 01 April 15
            if (viewHeight > ScreenFrame.size.width)
            {
                viewHeight = ScreenFrame.size.width;
            }
            
            float cellSize = viewHeight/(2.0);
            
            [cell setFrame:CGRectMake(0, 0, cellSize, cellSize)];
            cell.center                   = cell.view_point;
            [((viewCell *)cell).image_view setCenter:CGPointMake(cell.frame.size.width/2, cell.frame.size.width/2)];
            
            cell.delegate                 = self;
            //NSLog(@"cell size: %f : %f",self.frame.size.width,self.frame.size.height);
...
- (void)cellTouchBegin:(SCHCircleViewCell *)cell
{
    ...
    if([_circle_view_delegate respondsToSelector:@selector(touchBeginCircleViewCell:indexOfCircleViewCell:)])
    {
        ...
        [_circle_view_delegate circleSetTitle:index];
    }
}
//by aswathy

- (void)cellTouchEndByIcon:(SCHCircleViewCell *)cell{
    // NSLog(@"cellTouchEndByIcon");
    ...
    switch (_touch_circle_style)
    {
        case SCHTouchCircleDefault:
            [self singletTapFromIconCircleDefault:_circle_cell_array touchCell:cell touchIndex:index];
            [_circle_view_delegate circleSetTitle:index];
            break;
        ...
        default:
            [self singleTapForGoToViewtouchIndex:index touchCell:cell];
            break;
    }
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag
{
    ...
    {
        --drag_animation_count;
        [_circle_view_delegate circleSetTitle:_current_index];
